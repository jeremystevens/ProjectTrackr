#!/usr/bin/env python3
"""
MySQL-only application for FlaskBin

A clean, minimal implementation that works with MySQL
without any PostgreSQL dependencies or conflicts.
"""
import os
import logging
...
# Import Flask packages before any SQLAlchemy or database code
...
from sqlalchemy import text, Column, Integer, String, Text, DateTime, Boolean, ForeignKey
...
def bulk_delete_expired_pastes():
    ...
    result = db.session.execute(
        text("DELETE FROM pastes WHERE expires_at IS NOT NULL AND expires_at < :now"),
        {'now': now}
    )
    db.session.commit()
