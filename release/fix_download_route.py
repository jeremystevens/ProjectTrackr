...
# Define the pattern for the download route's encryption handling
...
# Define the replacement for the download route
replacement = """    # Handle encrypted content
    content = paste.content
    
    # Add debug logging
    import logging
    logging.debug(f"DOWNLOAD: Handling encrypted paste: {paste.short_id}, Encrypted: {paste.is_encrypted}, Method: {paste.encryption_method}")
    ...
    if paste.is_encrypted:
        ...
        if paste.encryption_method == 'fernet-random' and not session.get('decrypted_pastes', {}).get(paste.short_id):
            ...
            logging.debug(f"DOWNLOAD: Random key from URL: {key}")
            ...
            logging.debug(f"DOWNLOAD: Using key from URL for paste {short_id}")
...
if matches:
    ...
    # We want to replace the second occurrence (download route)
    if len(matches) >= 2:
        match = matches[1]
        ...
        print("Successfully updated download route")
    else:
